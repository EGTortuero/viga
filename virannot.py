...
for newfile in sorted(glob.glob("CONTIG_*.fna")):
	...
	print "Running BLAST to predict the genes according to homology inference in %s" % newfile
	if args.blastexh==True:
		print "Using exhaustive mode (see Fozo et al. (2010) Nucleic Acids Res for details)."
		blastp_cline = NcbiblastpCommandline(query="temp.faa", db=args.blastdatabase, max_target_seqs=10, out="temp_blast.csv", outfmt='"6 qseqid sseqid pident length qlen slen qstart qend evalue bitscore stitle"', evalue=args.blastevalue, num_threads=args.ncpus, word_size=2, gapopen=8, gapextend=2, matrix="PAM70", comp_based_stats='0')
		stdout, stderr = blastp_cline()
	else:
		print "Using default parameters."
		blastp_cline = NcbiblastpCommandline(query="temp.faa", db=args.blastdatabase, max_target_seqs=10, out="temp_blast.csv", outfmt='"6 qseqid sseqid pident length qlen slen qstart qend evalue bitscore stitle"', evalue=args.blastevalue, num_threads=args.ncpus)
		stdout, stderr = blastp_cline()
	...
	print "Running HHPRED to predict the proteins according to HMM-HMM comparisons in %s" % newfile
	for singleprot in sorted(glob.glob("SEQ_*.faa")):
		print "Running %s" % singleprot
		hhout = "%s.hhr" % singleprot
		if args.hhsearchexh==True:
			print "Using exhaustive mode (see Fidler et al. (2016) Traffic for details)."
			subprocess.call(['hhblits', '-i', singleprot, '-d', args.hhblitsdatabase, '-o' , '/dev/null', '-oa3m', 'temp.a3m', '-n', '8', '-e', '0.001', '-E', '0.01', '-maxfilt', str(args.maxfilt), '-neffmax', str(args.neffmax),'-cpu', str(args.ncpus), '-v', "0"])
			subprocess.call(['hhsearch', '-i', 'temp.a3m', '-d', hh_search_dbs, '-o', hhout, '-mac', '-e', '0.01', '-cpu', str(args.ncpus), '-v', "0"])
			os.remove("temp.a3m")
		else:
			print "Using default parameters."
			subprocess.call(['hhblits', '-i', singleprot, '-d', args.hhblitsdatabase, '-o' , '/dev/null', '-oa3m', 'temp.a3m', '-n', '3', '-E', '0.5', '-maxfilt', str(args.maxfilt), '-neffmax', str(args.neffmax), '-cpu', str(args.ncpus), '-v', "0"])
			subprocess.call(['hhsearch', '-i', 'temp.a3m', '-d', hh_search_dbs, '-o', hhout, '-cpu', str(args.ncpus), '-v', "0"])
			os.remove("temp.a3m")
	print "Done. HHSEARCH was done to predict the function of the genes by HMM-HMM comparisons\n\n"
